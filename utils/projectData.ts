  ...
  <g>
    ...
    <g>
      ...
      <path class="st0" d="M1380.81,460.07c-13.91-.95-103.66,26.47-108.74,35.15-6.65,43.04-20.37,84.6-27.94,127.42-3.6,20.34-13.02,66.72-12.88,84.45.11,14.64,26.27,51.86,43.75,47.77l130.53-54.3c4.6-13.88-6.56-35.84-21.32-37.32-14.41-1.44-90.69,37.98-98.47,30.11-.09-3.22-.24-6.48-.15-9.74.18-6.65,6.12-40.53,8.8-43.71,8.15-9.66,83.33-20.56,87.93-34.6,3.32-14.8-5.37-28.32-17.77-35.64l-59.37,18.82,13.68-62.02c28.76-16.26,62.56-18.27,94.17-24.23,11.96-21.54-11-40.73-32.21-42.17Z"/>
          ...
          return (
            <div className="min-h-screen flex flex-col p-3 bg-glez-dark selection:bg-glez-lime selection:text-black font-sans relative">
                ...
                {tools.map((tool) => {
                    ...
                    return (
                      <button key={tool.id} onClick={() => runAeScript(tool.functionName)} onMouseEnter={() => setHoveredId(tool.id)} onMouseLeave={() => setHoveredId(null)}
                        className="group relative flex flex-col items-start justify-between p-3 rounded-xl border border-zinc-700/50 bg-glez-card transition-all duration-300 ease-out hover:border-glez-lime hover:shadow-[0_0_15px_rgba(204,255,0,0.1)] hover:-translate-y-1 h-32 overflow-hidden">
                        
                        <div className="flex items-start justify-between w-full mb-2">
                            <div className="p-2 rounded-lg bg-zinc-800 transition-colors duration-300 group-hover:bg-glez-lime group-hover:text-black text-zinc-300 border border-white/5">
                                <Icon size={20} />
                            </div>
                        ...
                        <div className="text-left z-10 w-full">
                            <h3 className="text-sm font-bold text-white group-hover:text-glez-lime transition-colors truncate">{tool.name}</h3>
                            <p className="text-[10px] text-zinc-500 mt-1 leading-tight line-clamp-2">{tool.description}</p>
                        </div>
              ...
              {/* Footer */}
              <footer className="mt-auto pt-3 border-t border-white/5 text-center"><p className="text-[9px] text-zinc-600 uppercase tracking-widest">Designed for professional workflows</p></footer>
            </div>
          );
        };
